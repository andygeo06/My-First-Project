...
<style>
    ...
    .section-header-strat {{ background-color: #1A365D; padding: 15px; border-radius: 10px 10px 0 0; text-align: center; border-bottom: 2px solid #1F6FEB; }}
    .section-header-core {{ background-color: #7B341E; padding: 15px; border-radius: 10px 10px 0 0; text-align: center; border-bottom: 2px solid #F56565; }}
    div[data-testid="stExpander"] {{ background-color: #161B22 !important; border: 1px solid #30363D !important; border-radius: 8px !important; margin-bottom: 12px; }}
    div[data-testid="stExpander"] div[role="region"] {{ background-color: #0D1117 !important; padding: 25px !important; border-top: 1px solid #30363D; }}
    div.stButton > button {{ background-color: #21262D; color: white; border: 1px solid #30363D; border-radius: 8px; transition: 0.3s; height: 3em; }}
...
def module_scorecard():
    try:
        ...
        st.error("Sheet 'Mod1_DD' not found."); return
    ...
    with st.expander("🔹 SI 3: Capital Formation"):
        ...
        stat = st.selectbox("Status", dd[stat_col].dropna().unique())
    ...
    with st.expander("🔸 CI 1: ER Turnaround Time (<4 hrs)"):
        c1, c2 = st.columns(2); ci1v = score_calc(c1.number_input("ER <4h", 0, key="ci1n"), c2.number_input("Total ER", 1, key="ci1d"), "ER TAT")
    with st.expander("🔸 CI 2: Discharge Turnaround (<6 hrs)"):
        c1, c2 = st.columns(2); ci2v = score_calc(c1.number_input("Disch <6h", 0, key="ci2n"), c2.number_input("Total Disch", 1, key="ci2d"), "Discharge TAT")
    with st.expander("🔸 CI 3: Lab Result Turnaround (<5 hrs)"):
        c1, c2 = st.columns(2); ci3v = score_calc(c1.number_input("Lab <5h", 0, key="ci3n"), c2.number_input("Total Lab", 1, key="ci3d"), "Lab TAT")
    with st.expander("🔸 CI 4: Healthcare Associated Infection Rate"):
        c1, c2 = st.columns(2); ci4v = score_calc(c1.number_input("HAI", 0, key="ci4n"), c2.number_input("D/D >48h", 1, key="ci4d"), "HAI Rate")
    with st.expander("🔸 CI 5: Client Experience Survey"):
        c1, c2 = st.columns(2); ci5v = score_calc(c1.number_input("Outstanding", 0, key="ci5n"), c2.number_input("Total Resp", 1, key="ci5d"), "Survey")
    with st.expander("🔸 CI 6: Disbursement Rate"):
        c1, c2 = st.columns(2); ci6v = score_calc(c1.number_input("Disbursed", 0.0, key="ci6n"), c2.number_input("Allocation", 1.0, key="ci6d"), "Disbursement")

    st.divider()
    h_name = st.text_input("Name of Head of Facility:")
    h_pos = st.text_input("Designation of Head of Facility:")

    res = {
        "SI1": s1, "SI2": s2, "SI3_Cat": cat, "SI3_Src": src, "SI3_Stat": stat, "SI4_Status": iso1, "SI4_Audit": iso2, 
        "SI5_24": pgs1, "SI5_25": pgs2, "SI6": s6v, "SI7": s7v, "SI8": s8v, "CI1": ci1v, "CI2": ci2v, "CI3": ci3v, "CI4": ci4v, "CI5": ci5v, "CI6": ci6v,
        "Head_Name": h_name, "Head_Pos": h_pos
    }

    b1, b2 = st.columns(2)
    if b1.button("🖨️ GENERATE REPORT & AUTO-SUBMIT", type="primary", use_container_width=True):
        submit_module_data(res, "Mod1"); st.session_state.show_print = True
    if b2.button("💾 SUBMIT DATA ONLY", use_container_width=True):
    ...
    <div style="font-family: Arial, sans-serif; padding: 40px; background: white; color: black; border: 2px solid #333; max-width: 800px; margin: 0 auto;">
        ...
        <table style="width: 100%; border-collapse: collapse; text-align: center;">
            <tr style="background:#1A365D; color:white;"><th colspan="2">I. STRATEGIC INDICATORS</th></tr>
            <tr><td>SI 1: PHU Functionality</td><td>{d['SI1']:.2f}%</td></tr>
            <tr><td>SI 3: Capital Formation</td><td>{d['SI3_Cat']} ({d['SI3_Stat']})</td></tr>
            <tr><td>SI 4: ISO Status</td><td>{d['SI4_Status']}</td></tr>
            <tr style="background:#7B341E; color:white;"><th colspan="2">II. CORE QUALITY INDICATORS</th></tr>
            <tr><td>CI 1: ER TAT (<4h)</td><td>{d['CI1']:.2f}%</td></tr>
            <tr><td>CI 6: Disbursement</td><td>{d['CI6']:.2f}%</td></tr>
        </table>
        <br><br>
        <table style="width:100%; text-align:center;">
            <tr><td><b>{u['user']}</b><br>{u['pos']}</td><td><b>{d['Head_Name']}</b><br>{d['Head_Pos']}</td></tr>
        </table>
        <center><br><button onclick="window.print()">Print to PDF</button></center>
    </div>"""
...
def login_screen():
    ...
    if "auth_mode" not in st.session_state:
        ...
        if c2.button("🔑 EXISTING", use_container_width=True, type="primary"): st.session_state.auth_mode = "existing"; st.rerun()
    else:
        ...
        if st.session_state.auth_mode == "new":
            ...
            u_name = st.text_input("Name"); u_pos = st.text_input("Designation")
