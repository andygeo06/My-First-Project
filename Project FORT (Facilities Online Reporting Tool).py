...
# --- 2. CSS ENGINE (Forcing Dark Mode & Expander Contrast) ---
st.markdown(f"""
<style>
    /* Force Fixed Dark Mode Backgrounds */
    .stApp {{
        background-color: #0E1117;
        color: #C9D1D9;
    }}
    
    /* Strategic & Core Section Headers */
    ...
    /* Expander Styling: Dark Header, Darker Content */
    div[data-testid="stExpander"] {{
        background-color: #161B22 !important; /* Header Background */
    ...
    div[data-testid="stExpander"] div[role="region"] {{
        background-color: #0D1117 !important; /* Inner Content Background (Contrast) */
...
def clean_pct(input_str):
    """Safely converts "85%" or "85" to 85.0"""
    try:
...
def score_calc(n, d, label):
    """Displays real-time math results"""
...
def module_scorecard():
    ...
    # --- STRATEGIC SECTION ---
    st.markdown('<div class="section-header-strat"><h2>📊 STRATEGIC PERFORMANCE</h2></div>', unsafe_allow_html=True)
    
    with st.expander("🔹 SI 1: % Functionality of PHU", expanded=True):
        s1_raw = st.text_input("Input Percentage (e.g., 90%)", value="0%", key="si1_in")
        s1 = clean_pct(s1_raw)
        st.caption(f"Captured Value: **{s1}%**")
        
    with st.expander("🔹 SI 2: Green Viability Assessment (GVA)"):
        s2_raw = st.text_input("GVA Score (e.g., 85.5%)", value="0%", key="si2_in")
        s2 = clean_pct(s2_raw)
        st.caption(f"Captured Value: **{s2}%**")
    ...
    with st.expander("🔹 SI 4 & 5: ISO & PGS Accreditation"):
        c1, c2 = st.columns(2)
        iso = c1.selectbox("ISO Status", dd["Indicator 4, DD1"].dropna().unique())
        pgs = c2.selectbox("2025 PGS Status", dd["Indicator 5, DD2"].dropna().unique())

    with st.expander("🔹 SI 6: Functional Specialty Centers"):
        c1, c2 = st.columns(2)
        s6v = score_calc(c1.number_input("Functional Centers", 0, key="s6n"), 
                         c2.number_input("Target Centers", 1, key="s6d"), "SI 6")

    with st.expander("🔹 SI 7: Zero Co-Payment Patients"):
        c1, c2 = st.columns(2)
        s7v = score_calc(c1.number_input("Zero Co-Pay Patients", 0, key="s7n"), 
                         c2.number_input("Total Patients", 1, key="s7d"), "SI 7")

    with st.expander("🔹 SI 8: Paperless EMR Areas"):
        c1, c2 = st.columns(2)
        s8v = score_calc(c1.number_input("Paperless Areas", 0, key="s8n"), 
                         c2.number_input("Expected Areas", 1, key="s8d"), "SI 8")
    ...
    with st.expander("🔸 CI 1: ER Turnaround Time (<4 hrs)"):
        c1, c2 = st.columns(2)
        ci1v = score_calc(c1.number_input("ER <4h Count", 0, key="ci1n"), 
                          c2.number_input("Total ER Patients", 1, key="ci1d"), "ER TAT")

    with st.expander("🔸 CI 2: Discharge Turnaround (<6 hrs)"):
        c1, c2 = st.columns(2)
        ci2v = score_calc(c1.number_input("Discharge <6h Count", 0, key="ci2n"), 
                          c2.number_input("Total Discharges", 1, key="ci2d"), "Discharge TAT")

    with st.expander("🔸 CI 5: Client Experience Survey"):
        c1, c2 = st.columns(2)
        ci5v = score_calc(c1.number_input("Outstanding Ratings", 0, key="ci5n"), 
                          c2.number_input("Total Respondents", 1, key="ci5d"), "Survey")

    with st.expander("🔸 CI 6: Disbursement Rate"):
        c1, c2 = st.columns(2)
        ci6v = score_calc(c1.number_input("Disbursement Amount", 0.0, key="ci6n"), 
                          c2.number_input("Total Allocation", 1.0, key="ci6d"), "Disbursement")
    ...
    if st.button("🖨️ GENERATE REPORT FOR PRINTING", type="primary", use_container_width=True):
        res = {
            "s1": s1, "s2": s2, "cat": cat, "stat": stat,
            "ci1": ci1v, "ci2": ci2v, "ci5": ci5v, "ci6": ci6v,
            "h_name": h_name, "h_pos": h_pos
        }
        generate_print_view(res)

# --- 5. REPORT ENGINE & LOGIN LOGIC ---
...
def generate_print_view(d):
    ...
    <div style="font-family: Arial; padding: 40px; background: white; color: black; border: 2px solid #333;">
        ...
        <p><b>PHU Functionality:</b> {d['s1']}% | <b>GVA Score:</b> {d['s2']}%</p>
        <p><b>Capital Formation:</b> {d['cat']} — Status: <b>{d['stat']}</b></p>
        <p><b>ER TAT:</b> {d['ci1']:.2f}% | <b>Experience:</b> {d['ci5']:.2f}% | <b>Disbursement:</b> {d['ci6']:.2f}%</p>
        <br><br>
        ...
        <br><button onclick="window.print()" style="padding:10px; background:#1A365D; color:white; border:none; cursor:pointer;">Print to PDF</button>
    </div>
    """
    ...
    if "auth_mode" not in st.session_state:
        ...
        if st.session_state.auth_mode == "new":
            ...
            if st.button("Register Profile"):
                ...
                st.success(f"Profile Created! Your ID: {new_id}")
                if st.button("Go to Dashboard"): st.rerun()

        elif st.session_state.auth_mode == "existing":
            uid = st.text_input("Enter your ID Code")
            if st.button("Enter Portal"):
                profiles = get_all_profiles()
                if uid in profiles["User_ID"].values:
                    row = profiles[profiles["User_ID"] == uid].iloc[0]
                    st.session_state.user_id = uid
                    st.session_state.user_info = {"hosp": row["Hospital_Name"], "level": row["Service_Capability"], "user": row["Encoder_Name"], "pos": row["Position"]}
                    st.rerun()
